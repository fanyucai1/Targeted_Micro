...
def bcftools(bam,ref_fasta,outdir,prefix):
    out=outdir+"/"+prefix
    cmd = "bcftools mpileup -Ou -f %s %s | bcftools call --ploidy 1 -mv -Oz -o %s.calls.vcf.gz" % (ref_fasta,bam, out)
    cmd += " && bcftools index %s.calls.vcf.gz" % (out)
    cmd += " && cat %s | bcftools consensus %s.calls.vcf.gz -p %s > %s.consensus.fa" % (ref_fasta, out,prefix, out)
