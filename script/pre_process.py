...
cmd=("java -jar /software/Trimmomatic-0.39/trimmomatic-0.39.jar PE %s %s "
...
cmd="/software/fastqtk/fastqtk interleave %s_R1.fq.gz %s_R1.fq.gz - | gzip > %s.interleave.fq.gz"%(outdir_1,outdir_1,outdir_2)
subprocess.check_call(cmd,shell=True)
## Human reads are removed with a modified version of the SRA Human Read Scrubber tool.
cmd="/software/sra-human-scrubber/scripts/scrub.sh -s -p 24 -i %s.qc.fq.gz|/software/fastqtk/fastqtk deinterleave - %s.R1.scrub.fq %s.R1.scrub.fq"%(outdir_2,outdir_2,outdir_2)
subprocess.check_call(cmd,shell=True)
...
cmd="/software/MEGAHIT-1.2.9-Linux-x86_64-static/bin/megahit -1 %s.R1.scrub.fq -2 %s.R2.scrub.fq -o %s"%(outdir_2,outdir_2,outdir_3)
subprocess.check_call(cmd,shell=True)
...
cmd="/software/cd-hit-v4.8.1-2019-0228/cd-hit-est -i /raw_data/%s -o /outdir/%s.fasta -c 0.95 -n 5 -g 1 -aS 0.8 -T 0"
#subprocess.check_call(cmd,shell=True)
